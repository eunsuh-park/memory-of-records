...
- 진행 로그:
  ...
  - 사용자 완료 260814 — 휴지통 DB 생성·환경변수 설정, 삭제 동작 확인. 복원 페이지는 네비 공간 이슈로 Backlog에만 수집
...
**사용자 전용** (에이전트가 Notion/시크릿에 대신 넣을 수 없음) → 「내가 할 일」:

| 항목 | P | 상태 |
|------|---|------|
| 기존 노트 `size` 데이터 채우기 | P1 | 남음 |
| 휴지통 DB 생성 · `NOTION_TRASH_DATABASE_ID` | P1 | **완료 260814** (삭제 동작 확인) |
| 구입처·블로그 링크 속성 | P3 | 남음 |
| 프로덕션 env/시크릿 점검 | — | 남음 |

> 노트 잠금 속성: 260812 사용자 요청으로 제외 (`Backlog` cancelled).  
> 휴지통 복원 페이지: 260814 백로그만 수집 (네비 메뉴 자리가 생기기 전).

## 내가 할 일

- [x] 휴지통 DB 생성 · `NOTION_TRASH_DATABASE_ID` 환경변수 (260814 확인)
- [ ] 기존 노트 Notion `size` 값 채우기 (비율 렌더 병목)
- [ ] 구입처·블로그 링크 속성 (P3, 속성 추가 후 UI)
- [ ] 프로덕션 env/시크릿 점검

## Done

- 포커스 패널 노트 삭제 → 휴지통 DB (260813 코드 · 260814 DB·env·동작 확인)
