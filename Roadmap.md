...
- 수락 기준:
  - [ ] 카톡·슬랙 등에 홈 링크를 붙이면 사이트 이름, 소개 문구, 기본 이미지가 보임
  - [ ] 공개 노트 공유 링크를 붙이면 노트 제목, 메모(없으면 소개 문구), 앞표지가 보임
  - [ ] 없거나 비공개 노트 링크는 사이트 기본 미리보기만 나가고, 내용이 새지 않음
  - [ ] 노트 페이지를 브라우저로 열면 탭 제목이 노트명으로 바뀜 (앱 레이아웃은 그대로)
- 진행 로그:
