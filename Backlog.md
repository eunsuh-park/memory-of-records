...
> 마지막 enrich: 260812 · 마지막 capture: 260814  
> Inbox 잔여: 0  
> 소스: Notion 백로그(정리 기준일 2026-08-07) → capture 260812 → enrich 260812 → capture 260814(휴지통 복원 페이지)  
> 코드 반영 참고: main에 PR #19–22 · #26(Favorites) · #27(Favorites UI) · #29(Bookmark Note) · #31(노트 삭제) 머지됨. 이번 정리에서 #23(모바일 인디케이터) 합침. 미머지 초안: PR #24(slug·JPG·비공개 업로드·2-page gap 등). #25·#28은 main에 흡수되어 폐기.
...
- 상태: done (코드 260813 · 휴지통 DB 생성·env 사용자 완료, 동작 확인 260814)
- 우선순위: P1
...
- 열린 질문: 삭제 일시(Date) 속성 추가 권장. 열람·복원 UI는 아래 항목으로 분리 (260814)
- 원문 메모: 「빨간원 위치에 삭제 아이콘 · Dialog 삭제/취소 · 휴지통 DB로 이동 · DB 아직 없음」
- (추가 260814) 사용자: 휴지통 DB 생성·환경변수 설정 완료, 삭제 동작 확인
- Backlog 참조: [Roadmap 포커스 패널 노트 삭제](Roadmap.md)

### 휴지통 노트 복원 여부 결정 페이지 `(수집 260814)`
- 상태: backlog
- 우선순위: P2 — 삭제·DB는 됐으나 **네비게이션 메뉴가 없어 넣을 공간이 없음**. PC 사이드네비(동일 그룹 P2) 이후
- 목적: 휴지통 DB에 있는 노트를 보고, 나중에 **복원할지 말지** 결정한다
- 화면/진입: 미정 — 현재 헤더/드로어에는 자리가 없음. 사이드네비(또는 동등한 메뉴)가 생긴 뒤 진입점 배치
- 시나리오:
  - 기본: 휴지통 목록에서 노트를 보고 복원 / 복원하지 않음(유지 또는 영구삭제)을 고름
  - 예외: 휴지통이 비어 있을 때. 복원 시 원래 노트북 DB·주크박스에 다시 보이는지
- 데이터/API: 휴지통 DB 읽기 · 복원(노트북 DB로 되돌리기) API는 아직 없음 — 확인 필요
- 디자인·UX: 네비에 넣을 자리가 나올 때까지 UI 구현하지 않음
- 열린 질문: 「복원할지 말지」가 복원 vs 휴지통에 두기인지, 영구삭제인지. 진입점이 사이드네비인지 설정인지
- 원문 메모: 「이 DB내의 노트를 나중에 복원할지 말지 결정하는 페이지 · 지금 네비게이션 메뉴가 없어서 넣을 공간이 안 나옴 · 일단 백로그」
- 의존: [PC 사이드 네비게이션](Backlog.md), [포커스 패널 노트 삭제](Backlog.md)
