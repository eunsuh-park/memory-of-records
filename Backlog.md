...
### 노트 공유 링크 SEO 메타 `(수집 260904)`
- 상태: done (260904)
- 우선순위: P1 — 링크는 복사되지만 카톡·슬랙 등에 붙이면 제목·이미지·설명이 비어 있음
- 목적: 노트 공유 URL을 메신저·SNS에 붙일 때 노트 제목, 대표 문구, 표지 이미지가 미리보기로 보이게 한다
- 화면/진입: `/note/{slug}` · `/note/{slug}?p=N` 을 카카오톡·슬랙·디스코드·X 등에 붙여넣을 때. 사이트 홈을 붙일 때는 기본 소개 문구·기본 이미지
- 시나리오:
  - 기본: 공개 노트 링크 → 미리보기 제목은 노트명, 설명은 노트 메모(없으면 사이트 소개), 이미지는 앞표지
  - 예외: 없거나 비공개 노트는 사이트 기본 메타만. Bookmark Note는 공유하지 않으므로 대상 아님. 크롤러는 JS를 실행하지 않으므로 서버에서 메타를 넣는다
- 데이터/API: Notion 공개 노트(제목·description·cover_front_url). 새 Notion 속성 없음. OG 이미지는 Cloudinary 표지를 1200×630 JPG로 맞춤
- 디자인·UX: 앱 화면 레이아웃은 그대로. 브라우저 탭 제목만 노트명으로 바뀜. 미리보기는 카톡 등 외부 앱에서 보임
- 열린 질문: 페이지 공유(`?p=`)일 때 미리보기 이미지를 해당 장 스캔으로 바꿀지 — 1차는 표지 유지
- 원문 메모: 「노트 공유 링크를 어딘가 붙일 때 들어갈 이미지나 대표 문구 등 SEO를 위한 메타 정보값들을 메꾸고 싶어. 할일을 계획하고 하나씩 해나가자.」
- Backlog 참조: [Roadmap 노트 공유 링크 SEO 메타](Roadmap.md)
