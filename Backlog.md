...
- [ ] **모바일 이미지 WebP 등 빠른 포맷으로 로드** `(수집 260906)`
  - 상태: roadmap (260906) — 묶음 「모바일 이미지 로드 최적화」
  - 요청 요지: 모바일에서 이미지를 PNG 원본 대신 WebP(사용자가 말한 wpng)처럼 용량이 작고 로드가 빠른 포맷으로 받게 한다.
  - 맥락/화면: 주크박스 표지 · 노트 뷰어 페이지 스캔. 코드에는 이미 `optimizeImageUrl`의 Cloudinary `f_auto`(webp/avif 자동) · `q_auto` · 너비 제한이 있다. 확장자만 바꾸는 게 아니라 딜리버리 URL 변환으로 처리하는 쪽.
  - 원문 메모: 「이미지 파일 확장자명을 wpng라던지 로드 빨리 되는 걸로 로드하도록 하기」
  - Backlog 참조: [Roadmap 모바일 이미지 로드 최적화](Roadmap.md)

- [ ] **같은 이미지를 매번 API로 다시 부르지 않게 캐시** `(수집 260906)`
  - 상태: roadmap (260906) — 묶음 「모바일 이미지 로드 최적화」
  - 요청 요지: 한 번 받은 이미지를 다시 볼 때 API/네트워크를 반복 호출하지 않는다.
  - 맥락/화면: 표지 목록(`noteCovers`)은 메모리 캐시가 있다. `fetchNotePages`와 Cloudinary pages API는 호출마다 다시 치고, 응답 헤더는 `Cache-Control: private, no-store`다. 뷰어는 세션 안 preload Map만 있다.
  - 원문 메모: 「캐시 사용해서 여러번 같은 이미지를 매번 API로 호출하지 않게 하기」
  - Backlog 참조: [Roadmap 모바일 이미지 로드 최적화](Roadmap.md)

- [ ] **이미지 로드 전 스켈레톤 UI** `(수집 260906)`
  - 상태: roadmap (260906) — 묶음 「모바일 이미지 로드 최적화」
  - 요청 요지: 아직 안 뜬 이미지를 빈 칸이 아니라 스켈레톤(임시 자리)으로 보여 준다.
  - 맥락/화면: 주크박스 카드는 로드 전 투명 픽셀, 뷰어는 Lottie 로딩. 스켈레톤 컴포넌트는 없음.
  - 원문 메모: 「스켈레톤 UI같은거로 로드되지 않은 이미지를 임시 이미지로 대체하기.」
  - Backlog 참조: [Roadmap 모바일 이미지 로드 최적화](Roadmap.md)
