...
| `GET /api/readNotebooks` | 시기/타입 목록·폼 옵션 (`view`: `period` \| `type` \| `formMeta`, period/type은 `visibility`) | `src/services/notionNotebooks.js` · `notionByType.js` · `createNote.js` |
...
| `POST /api/writeCovers` | 표지 앞·뒤 Cloudinary 업로드 | `src/services/createNote.js` |
