 ...
 *
 * Query: op=list | meta | hidden | bookmarked | bookmarkNotes
...
import { handleReadBookmarkNotes } from './_lib/handlers/bookmarkNotesStore.js';
...
export default async function handler(req, res) {
  ...
  if (op === 'bookmarkNotes') return handleReadBookmarkNotes(req, res);

  return res.status(400).json({
    error: 'Validation failed',
    message: "op은 'list' | 'meta' | 'hidden' | 'bookmarked' | 'bookmarkNotes' 중 하나여야 합니다"
  });
}
