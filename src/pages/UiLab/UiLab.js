...
const RESPONSIVE_MATRIX = [
  ...
  },
  {
    ...
    mobile: [
      ...
      '높이 ≤640px에서도 동일(햄버거 + 우측 드로어)'
    ],
    tablet: [
      '너비가 769–1024px이고 높이가 641px 이상이면 데스크톱 한 줄 레이아웃 유지',
      '≤1024px에서 「새 노트 추가」 라벨 숨김 · 44px + 아이콘만',
      '높이 ≤640px이면 너비와 무관하게 모바일과 같은 햄버거·우측 드로어'
    ],
    desktop: [
  ...
  },
  {
    ...
    tablet: [
      '너비 769–1024px·높이 641px 이상은 데스크톱과 같고 칩이 넘치면 수평 스크롤',
      '칩 모양은 FilterChip PC(가로 pill)를 그대로 씀',
      '높이 ≤640px이면 필터를 햄버거 헤더 바로 아래로 내린다'
    ],
    desktop: [
      ...
      <div class="ui-lab__inner">
        ...
        <section class="ui-lab__section" id="filter">
          ...
          <p class="ui-lab__section-desc">
            ...
            모바일(너비 ≤768px)과 낮은 화면(높이 ≤640px)에서는 page-header(로고·햄버거) 바로 아래에 고정됩니다.
