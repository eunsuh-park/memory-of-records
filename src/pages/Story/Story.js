/**
 * Story 페이지 (하드코딩 단일 페이지)
 * - StoryDetail.css 책형 레이아웃 사용
 * - HTML 구조는 아래 주석 참고하여 하드코딩
 */

import { router } from '../../router.js';
import './Story.css';
import '../StoryDetail/StoryDetail.css';
...
export async function renderStory() {
   ...
   * - 이미지: .story-detail-image 안에 <img> 또는 .story-placeholder-image
   * - 제목/부제목/날짜: .story-detail-header 안
   * - 본문: .story-detail-content 안
   * - 이미지 경로는 src/assets 에 넣고 경로 수정
   ============================================================ */

  const imageSrc = null; // TODO: 예) import myImage from '../../assets/your-image.jpg';
  const title = '제목'; // TODO: 하드코딩
  const subtitle = '';   // TODO: 하드코딩 (없으면 빈 문자열)
  const publishDate = ''; // TODO: 예) '2025.01.01'
    ...
    <main class="story-detail-main">
      <div class="story-book">
        ...
        <div class="story-book-content">
          <!-- [왼쪽: 이미지 영역] - 이미지 src 수정 또는 placeholder 유지 -->
          ...
          <div class="story-book-content-R">
            ${renderButton({ variant: 'backInline', ariaLabel: '이전 페이지로 돌아가기' })}
            <div class="story-detail-header">
              <h1 class="story-detail-title">${title}</h1>
              ${subtitle ? `<div class="story-detail-subtitle">${subtitle}</div>` : ''}
              <div class="story-detail-date">${publishDate}</div>
            </div>
