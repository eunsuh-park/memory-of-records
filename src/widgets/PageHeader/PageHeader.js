 ...
 * 모바일·낮은 화면(높이 ≤640px 또는 너비 ≤768px): 로고(좌) | 햄버거(우) + 우측 드로어
...
import { COMPACT_HEADER_MQ } from '../../utils/breakpoints.js';
...
function isNotesPath(path) {
  return (
    ...
    path.startsWith(PAGE_SCRAP_PATH) ||
...
function isFavoritesPath(path) {
  return path.startsWith(FAVORITES_PATH);
}

function isPageScrapPath(path) {
  return path.startsWith(PAGE_SCRAP_PATH);
}
...
export function renderPageHeader() {
  ...
  const pageScrapActive = isPageScrapPath(currentPath);
      ...
      <nav class="nav-drawer__nav">
        <div class="nav-drawer__group ${notesActive ? 'is-active' : ''}" role="group" aria-label="Notes">
          ...
          <div class="nav-drawer__sub">
            ...
            >Favorite</a>
            <a
              href="${PAGE_SCRAP_PATH}"
              class="nav-drawer__link ${pageScrapActive ? 'active' : ''}"
              data-link
              data-drawer-close
            >Page Scrap</a>
          </div>
  ...
  if (!window.__pageHeaderCompactMqBound) {
    window.__pageHeaderCompactMqBound = true;
    const compactMq = window.matchMedia(COMPACT_HEADER_MQ);
